...
if (isset($_POST['update'])) {
    ...
    $sql = $conn->prepare("UPDATE bansmenu SET fname = :fname,description =:description,details=:details,img =:img WHERE id= :id");
    $sql->bindParam(":id", $id);
...
<body>
    ...
    <div class="container mt-5">
        ...
        <form action="editadmin.php" method="POST" enctype="multipart/form-data">
            ...
            if (isset($_GET['id'])) {
                $id = $_GET['id'];
                $stmt = $conn->query("select * from bansmenu where id = $id");
                $stmt->execute();
                $data = $stmt->fetch();
            }
            ?>
            <div class="mb-3">
                ...
                <input type="text" value="<?= $data['fname']; ?>" require class="form-control" name="fname">
                <input type="hidden" value="<?= $data['img']; ?>" require class="form-control" name="img2">
            </div>
            <div class="mb-3">
                <label for="description" class="col-form-label">Description:</label>
                <input type="text" value="<?= $data['description']; ?>" require class="form-control" name="description">
            </div>
            <div class="mb-3">
                <label for="details" class="col-form-label">details:</label>
                <input type="text" value="<?= $data['details']; ?>" require class="form-control" name="details">
            </div>
            <div class="mb-3">
